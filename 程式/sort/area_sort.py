...
cursor.execute("SELECT member_search_record.account,member_search_record.video_id,video.video_name,area.area_name FROM new_schema1.member_search_record  left join video on video.video_id=member_search_record.video_id  left join vtype_record on vtype_record.video_id=video.video_id  left join area on video.area_id=area.area_id")
...
cursor.execute("SELECT area_name FROM new_schema1.area")
...
for k, v in finaldict.items(): 
  ...
  # 插入一條記錄
    sql = "SELECT  *  FROM  new_schema1.area_sort where  account = '{}' ;".format(k)
    try:
        ...
        if row is not None:
          ...
          for x in v:
            print( "insert into new_schema1.area_sort account,area_name,sort values (%s, %s,%s")
            val = (k,x[0],x[1])
            print(val)
            cursor.execute("insert into new_schema1.area_sort (account,area_name,sort) values (%s, %s,%s)",val)
            mydb.commit()
            print('成功新增資料')
